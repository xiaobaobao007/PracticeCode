...
public class Example extends Application implements Runnable {
	...
	public void start(Stage stage) throws Exception {
		...
		Button btn_sent = new Button(" 发送S ");
		btn_sent.setOnAction(this::btnSend);
		btn_sent.setPrefSize(60, 30);
		Button btn_close = new Button(" 关闭C ");
		btn_close.setPrefSize(60, 30);
		Button btn_upfile = new Button("上传文件");
		Button btn_uppicture = new Button("上传图片");
		Button btn_write = new Button("文档编辑");
		...
		Button btn_add = new Button("加入群聊");
		Button btn_choose = new Button("选择聊天对象");
		...
		TextArea ta_number = new TextArea();
		...
		AnchorPane.setTopAnchor(tf_addr, 30.0);
		AnchorPane.setLeftAnchor(tf_addr, 580.0);
		AnchorPane.setTopAnchor(btn_con, 30.0);
		AnchorPane.setLeftAnchor(btn_con, 750.0);

		AnchorPane.setTopAnchor(pane_03, 150.0);
		AnchorPane.setLeftAnchor(pane_03, 580.0);

		AnchorPane.setTopAnchor(tf_choose, 430.0);
		AnchorPane.setLeftAnchor(tf_choose, 580.0);
	...
	private void btnChooseHandler(ActionEvent event) {
		ChoiceDialog<String> dialog = new ChoiceDialog<>("", list);// 选择框
